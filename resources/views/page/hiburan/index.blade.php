        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="gap-5 items-start flex">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg w-1/2 p-4">
                    ...
                    <div>
                        <form class="max-w-sm mx-auto" method="POST" action="{{ route('hiburan.store') }}">
                            @csrf
                            <div class="mb-5">
                                <label for="type_hiburan"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Type Hiburan
                                </label>
                                <input type="text" name="type_hiburan"
                            ...
                            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    @php
                                        $no = 1;
                                    @endphp
                                    @foreach ($hiburans as $index => $hiburan)
                                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 px-4"
                                            ...
                                            <th scope="row"
                                                ...
                                                {{ ($hiburans->currentPage() - 1) * $hiburans->perPage() + $loop->iteration  }}
                                            </th>
                                            <td class="px-5 py-3">
                                            ...
                                            <td class="px-5 py-3">
                                                ...
                                                <button
                                                    ...
                                                    onclick="return hiburanDelete('{{ $hiburan->id }}','{{ $hiburan->type_hiburan }}')">
                    ...
                    <div class="flex flex-col  p-4 space-y-6">
                        <div class="">
                            <label for="text" class="block mb-2 text-sm font-medium text-gray-900">Type
                                Hiburan</label>
                            <input type="text" id="type_hiburan" name="type_hiburan"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
...
<script>
    const editSourceModal = (button) => {
        ...
        const type_hiburan = button.dataset.type_hiburan;
        const deskripsi = button.dataset.deskripsi;
        const harga = button.dataset.harga;
        let url = "{{ route('hiburan.update', ':id') }}".replace(':id', id);

        let status = document.getElementById(modalTarget);
        document.getElementById('title_source').innerText = `UPDATE ${type_hiburan}`;

        document.getElementById('type_hiburan').value = type_hiburan;
    ...
    const hiburanDelete = async (id, type_hiburan) => {
        let tanya = confirm(`Apakah anda yakin untuk menghapus hiburan ${type_hiburan} ?`);
        if (tanya) {
            await axios.post(`/hiburan/${id}`, {
                    '_method': 'DELETE',
